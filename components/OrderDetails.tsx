...
export default function OrderDetails() {
  ...
  const setDisabledPickupDates = (date: Date) => {
    ...
    const tomorrowsDate = new Date(currentDate.getTime() + 24 * 60 * 60 * 1000);
    return (
      date < tomorrowsDate ||
  ...
  const buttonDetails = "Review and send order";
  ...
  return (
    <div className="pb-6">
      <div className="mb-6">
        <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
          <PopoverTrigger asChild>
            ...
            >
              ...
              {date ? format(date, "PPP") : <span>Select a pickup date</span>}
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-auto p-0">
            <Calendar
              ...
              selected={date}
              onSelect={(e) => {
                cartStore.setPickupDate(format(e as Date, "PPP"));
                setDate(e);
        ...
        {cartStore.pickupDate && (
          <div className="p-4 bg-bb-brown-10 rounded-2xl mt-3 border border-bb-brown-20">
            <strong className="text-bb-ink">Selected pickup date</strong>:{" "}
            {cartStore.pickupDate}
          </div>
        )}
      </div>
      ...
      {canProceed && (
        <Button
          variant="ghost"
          onClick={() => setCheckoutState("confirmOrder")}
