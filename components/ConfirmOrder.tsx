...
export default function ConfirmOrder() {
  ...
  const { checkoutError, setCheckoutError, setCheckoutState, setShowSidebar } =
      ...
      {!isSent ? (
        <>
          ...
          <div className="p-5 bg-bb-brown-10 rounded-2xl my-6 border border-bb-brown-20">
            ...
            <p>
              <span className="font-semibold">Amount due on pickup:</span>{" "}
          ...
          <div className="sticky bottom-0 left-0 right-0 shadow-lg">
            ...
            >
              {isSendingOrder ? "Order is processing" : "Send order"}
              {isSendingOrder && (
                <svg
        ...
        <div className="bg-bb-sage/40 p-4 rounded-2xl border border-bb-sage">
          <h2 className="font-display text-2xl text-bb-ink">Order Sent!</h2>
          <p>
            Your order has been sent. You will receive an email confirmation
            shortly!
          </p>
        </div>
      )}
